...
const ServicesComponent = () => {
  ...
  return (
    <div className="h-full w-full p-2">
      ...
      {isLoading
        ? services.map((_, index) => <CardsLoader key={index} />)
        : services?.map((item) => (
            <div
              key={item._id}
              className={`flex flex-col justify-center items-start p-7 h-full w-full mb-10 border-[#191A23] border-b-4 border rounded-[20px] relative right-[1000px] window-animation-${item.title.replace(
                /\s/g,
                ""
              )}`}
            >
